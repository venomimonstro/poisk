...
export default function ReviewPanel({placeID}:{placeID:number}){
 const[stats,setStats]=useState<Stats>({review_count:0,average_rating:0});const[reviews,setReviews]=useState<Review[]>([]);const[permissions,setPermissions]=useState<Permissions|null>(null);const[csrf,setCSRF]=useState("");const[rating,setRating]=useState(5);const[body,setBody]=useState("");const[replyTo,setReplyTo]=useState<number|null>(null);const[replyBody,setReplyBody]=useState("");const[message,setMessage]=useState("");
 async function load(){const[s,list]=await Promise.all([json<Stats>(`/api/reviews/places/${placeID}/rating`),json<{reviews:Review[]}>(`/api/reviews/places/${placeID}/reviews?limit=20`)]);setStats(s);setReviews(list.reviews);try{await json("/api/account/me");const[c,p]=await Promise.all([json<{csrf_token:string}>("/api/account/csrf"),json<Permissions>(`/api/reviews/places/${placeID}/permissions`)]);setCSRF(c.csrf_token);setPermissions(p)}catch{setCSRF("");setPermissions(null)}}
 useEffect(()=>{setBody("");setMessage("");void load().catch(()=>setMessage("Отзывы временно недоступны"))},[placeID]);
 async function submit(e:FormEvent){e.preventDefault();if(!csrf){setMessage("Войдите в аккаунт, чтобы оставить отзыв");return}try{await json(`/api/reviews/places/${placeID}/review`,{method:"PUT",headers:{"Content-Type":"application/json","X-CSRF-Token":csrf},body:JSON.stringify({rating,body})});setBody("");setMessage("Отзыв сохранён");await load()}catch(err){setMessage(err instanceof Error?err.message:"Не удалось сохранить отзыв")}}
 async function remove(){if(!csrf||!permissions?.my_review_id)return;try{await fetch(`/api/reviews/reviews/${permissions.my_review_id}`,{method:"DELETE",credentials:"same-origin",headers:{"X-CSRF-Token":csrf}});setMessage("Отзыв удалён");await load()}catch{setMessage("Не удалось удалить отзыв")}}
 async function report(reviewID:number){if(!csrf){setMessage("Войдите в аккаунт, чтобы пожаловаться");return}try{await json(`/api/reviews/reviews/${reviewID}/report`,{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-Token":csrf},body:JSON.stringify({reason:"OTHER",details:"Жалоба пользователя из карточки организации"})});setMessage("Жалоба отправлена")}catch(err){setMessage(err instanceof Error?err.message:"Не удалось отправить жалобу")}}
 async function reply(reviewID:number){if(!csrf||!permissions?.can_reply)return;try{await json(`/api/reviews/reviews/${reviewID}/reply`,{method:"PUT",headers:{"Content-Type":"application/json","X-CSRF-Token":csrf},body:JSON.stringify({body:replyBody})});setReplyBody("");setReplyTo(null);setMessage("Ответ владельца сохранён");await load()}catch(err){setMessage(err instanceof Error?err.message:"Не удалось сохранить ответ")}}
