...
RETURNING ws.sitemap_id,ws.site_id,s.domain_id,s.host,ws.sitemap_url,ws.depth,ws.attempts,ws.max_attempts,ws.worker_id`
	...
	for rows.Next(){var task SitemapTask;if err:=rows.Scan(&task.ID,&task.SiteID,&task.DomainID,&task.Host,&task.URL,&task.Depth,&task.Attempts,&task.MaxAttempts,&task.WorkerID);err!=nil{return nil,err};out=append(out,task)}
	return out,rows.Err()
}
...
SET status=CASE WHEN attempts>=max_attempts THEN 'FAILED' ELSE 'RETRY' END,
    ...
    lease_until=NULL,worker_id=NULL,last_error=COALESCE(last_error,'lease_expired'),updated_at=now()
...
func (r *Repository) CompleteSitemap(ctx context.Context,task SitemapTask) error {
	tag,err:=r.db.Exec(ctx,`UPDATE webmaster_sitemaps SET status='FETCHED',lease_until=NULL,worker_id=NULL,last_error=NULL,updated_at=now() WHERE sitemap_id=$1 AND status='LEASED' AND worker_id=$2`,task.ID,task.WorkerID)
	if err!=nil{return err};if tag.RowsAffected()!=1{return errors.New("sitemap lease ownership lost")};return nil
}

func (r *Repository) RetrySitemap(ctx context.Context,task SitemapTask,cause error,retryAfter time.Duration) error {
	if retryAfter<0{retryAfter=0};message:="sitemap processing failed";if cause!=nil{message=cause.Error()};if len(message)>2048{message=message[:2048]}
	tag,err:=r.db.Exec(ctx,`
UPDATE webmaster_sitemaps
SET status=CASE WHEN attempts>=max_attempts THEN 'FAILED' ELSE 'RETRY' END,
...
func (r *Repository) AddChildSitemaps(ctx context.Context,task SitemapTask,urls []string) error {
	if len(urls)==0{return nil};if task.Depth>=8{return ErrInvalidInput}
	tx,err:=r.db.BeginTx(ctx,pgx.TxOptions{});if err!=nil{return err};defer func(){_=tx.Rollback(ctx)}()
	allowed,err:=sitemapTaskAllowed(ctx,tx,task);if err!=nil{return err};if !allowed{return ErrSiteBlocked}
...
ON CONFLICT(site_id,sitemap_url) DO UPDATE SET
  ...
  available_at=CASE WHEN webmaster_sitemaps.status='DELETED' THEN now() ELSE webmaster_sitemaps.available_at END,
...
ON CONFLICT(normalized_url) DO UPDATE SET
    next_crawl_at=LEAST(COALESCE(urls.next_crawl_at,now()),now()),
...
RETURNING url_id,version,domain_id`,task.DomainID,raw).Scan(&urlID,&version,&domainID)
		if err!=nil{return fmt.Errorf("ensure sitemap URL: %w",err)};if domainID!=task.DomainID{return ErrNotFound}
		if _,err:=tx.Exec(ctx,`
INSERT INTO crawl_queue(url_id,domain_id,generation,priority,status,available_at)
SELECT $1,$2,$3,80,'READY',now() WHERE EXISTS(SELECT 1 FROM urls WHERE url_id=$1 AND crawl_status<>'BLOCKED')
...
func sitemapTaskAllowed(ctx context.Context,tx pgx.Tx,task SitemapTask)(bool,error){
	var allowed bool
	err:=tx.QueryRow(ctx,`
SELECT EXISTS(
  SELECT 1 FROM webmaster_sites s JOIN domains d ON d.domain_id=s.domain_id
  WHERE s.site_id=$1 AND s.domain_id=$2 AND s.status='VERIFIED' AND d.status='ACTIVE' AND d.policy<>'BLOCK'
)`,task.SiteID,task.DomainID).Scan(&allowed)
	return allowed,err
}
