...
func (h Handler) Routes()http.Handler{
	...
	r.Get("/websites",h.Websites)
	r.Get("/trends",h.Trends)
	r.Get("/sitemap",h.Sitemap)
	r.Get("/sitemap/stats",h.SitemapStats)
	r.Get("/sitemap/shard",h.SitemapShard)
	return r
}
...
func (h Handler) Sitemap(w http.ResponseWriter,r *http.Request){if !h.available(w){return};after,ok:=intParam(r,"after",0);if !ok{writeError(w,400,"invalid_after");return};limit,ok:=intParam(r,"limit",200);if !ok||limit<1||limit>500{writeError(w,400,"invalid_limit");return};items,next,err:=h.Hub.Sitemap(r.Context(),int64(after),limit);if err!=nil{writeError(w,503,"datahub_error");return};writeJSON(w,200,map[string]any{"items":items,"next_after":next})}
func (h Handler) SitemapStats(w http.ResponseWriter,r *http.Request){if !h.available(w){return};size,ok:=intParam(r,"size",500);if !ok||size<100||size>5000{writeError(w,400,"invalid_size");return};stats,err:=h.Hub.SitemapStats(r.Context(),size);if err!=nil{writeError(w,503,"datahub_error");return};writeJSON(w,200,stats)}
func (h Handler) SitemapShard(w http.ResponseWriter,r *http.Request){if !h.available(w){return};shard,ok:=intParam(r,"shard",0);if !ok||shard>100000{writeError(w,400,"invalid_shard");return};size,ok:=intParam(r,"size",500);if !ok||size<100||size>5000{writeError(w,400,"invalid_size");return};items,err:=h.Hub.SitemapShard(r.Context(),shard,size);if err!=nil{writeError(w,503,"datahub_error");return};writeJSON(w,200,map[string]any{"items":items,"shard":shard,"size":size})}
...
func writeJSON(w http.ResponseWriter,status int,v any){w.Header().Set("Content-Type","application/json; charset=utf-8");if w.Header().Get("Cache-Control")==""{w.Header().Set("Cache-Control","public, max-age=60, stale-while-revalidate=300")};w.Header().Set("X-Content-Type-Options","nosniff");w.WriteHeader(status);_ = json.NewEncoder(w).Encode(v)}
