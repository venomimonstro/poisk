...
func (s *Source) Load(ctx context.Context,placeID,version int64)(indexmanticore.OrganizationDocument,error){
	...
	err:=s.db.QueryRow(ctx,`SELECT place_id,version,name,COALESCE(address,''),COALESCE(category_key,''),COALESCE(phone,''),COALESCE(website,''),
normalized_name,COALESCE(normalized_address,''),latitude,longitude,quality_score,source_count,status
FROM organizations WHERE place_id=$1`,placeID).Scan(&doc.ID,&doc.EntityVersion,&doc.Name,&doc.Address,&doc.CategoryKey,&doc.Phone,&doc.Website,&doc.NormalizedName,&doc.NormalizedAddress,&latitude,&longitude,&doc.QualityScore,&doc.SourceCount,&doc.Status)
...
func (s *Source) RebuildBatch(ctx context.Context,afterPlaceID int64,limit int)([]indexmanticore.OrganizationDocument,error){
	...
	rows,err:=s.db.Query(ctx,`SELECT place_id,version,name,COALESCE(address,''),COALESCE(category_key,''),COALESCE(phone,''),COALESCE(website,''),
normalized_name,COALESCE(normalized_address,''),latitude,longitude,quality_score,source_count,status
FROM organizations WHERE place_id>$1 AND status IN ('ACTIVE','REVIEW') ORDER BY place_id LIMIT $2`,afterPlaceID,limit)
	...
	for rows.Next(){
		...
		if err:=rows.Scan(&doc.ID,&doc.EntityVersion,&doc.Name,&doc.Address,&doc.CategoryKey,&doc.Phone,&doc.Website,&doc.NormalizedName,&doc.NormalizedAddress,&lat,&lon,&doc.QualityScore,&doc.SourceCount,&doc.Status);err!=nil{return nil,err}
