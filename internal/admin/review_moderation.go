...
func (s Service) ApplyReviewModeration(ctx context.Context,session Session,token string)(ReviewModerationRow,error){
	...
	nextStatus:="VISIBLE";eventAction:="SHOW";reportStatus:="DISMISSED";if p.Action=="HIDE"{nextStatus="HIDDEN";eventAction="HIDE";reportStatus="RESOLVED"};if p.Action=="REJECT"{nextStatus="REJECTED";eventAction="REJECT";reportStatus="RESOLVED"}
	...
	if _,err=tx.Exec(ctx,`UPDATE organization_review_reports SET status=$2,resolved_at=now(),resolved_by_admin_id=$3 WHERE review_id=$1 AND status='OPEN'`,reviewID,reportStatus,session.AdminID);err!=nil{return ReviewModerationRow{},err}
