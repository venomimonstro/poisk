...
func (r *Repository) Ensure(ctx context.Context,userID,siteID int64)(Config,error){
 ...
 SELECT s.site_id,s.origin,s.host FROM webmaster_sites s JOIN domains d ON d.domain_id=s.domain_id
...
FROM upserted u JOIN owned o ON o.site_id=u.site_id`,userID,siteID,key).
		Scan(&out.WidgetID,&out.SiteID,&out.PublicKey,&out.Origin,&out.Host,&out.Status,&out.MaxResults,&out.CreatedAt)
...
SELECT c.widget_id,c.site_id,c.public_key,o.origin,o.host,c.status,c.max_results,c.created_at FROM changed c JOIN owned o ON o.site_id=c.site_id`,userID,siteID,key).
		Scan(&out.WidgetID,&out.SiteID,&out.PublicKey,&out.Origin,&out.Host,&out.Status,&out.MaxResults,&out.CreatedAt)
...
func (r *Repository) Owned(ctx context.Context,userID,siteID int64)(Config,error){
	var out Config
	err:=r.db.QueryRow(ctx,`SELECT w.widget_id,w.site_id,w.public_key,s.origin,s.host,w.status,w.max_results,w.created_at
