...
func (r Repository) LeaseOutbound(ctx context.Context,worker string,limit int,lease time.Duration,now time.Time)([]OutboundDelivery,error){
	if r.DB==nil||strings.TrimSpace(worker)==""||lease<=0{return nil,ErrInvalid};if limit<=0{limit=20};if limit>100{limit=100};if now.IsZero(){now=time.Now().UTC()};leaseSeconds:=int64(lease/time.Second);if leaseSeconds<1{leaseSeconds=1}
...
func (r Repository) RetryOutbound(ctx context.Context,deliveryID int64,worker,code,detail string,now time.Time)error{
	...
	_,err=tx.Exec(ctx,`UPDATE mail_outbound_deliveries SET status=$3,lease_owner=NULL,lease_until=NULL,next_attempt_at=$4,last_error_code=NULLIF($5,''),last_error_detail=NULLIF($6,''),updated_at=$7 WHERE delivery_id=$1 AND lease_owner=$2`,deliveryID,worker,status,now.Add(delay),code,detail,now);if err!=nil{return err}
