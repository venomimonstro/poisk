...
func (s InboundStore) ResolveRecipient(ctx context.Context,raw string)(int64,error){
	...
	var mailboxID int64
	err=s.Repo.DB.QueryRow(ctx,`SELECT a.mailbox_id FROM mail_external_aliases a JOIN mailboxes m ON m.mailbox_id=a.mailbox_id WHERE a.address=$1 AND a.status='ACTIVE' AND m.status='ACTIVE'`,address).Scan(&mailboxID)
	if errors.Is(err,pgx.ErrNoRows){return 0,ErrNotFound};return mailboxID,err
}
...
func (s InboundStore) Ingest(ctx context.Context,eventID,recipient string,raw []byte,now time.Time)(messageID int64,duplicate bool,err error){
	...
	var existingDigest,existingStatus string;var existingMessage *int64
	err=s.Repo.DB.QueryRow(ctx,`SELECT body_sha256,status,message_id FROM mail_inbound_receipts WHERE event_id=$1`,eventID).Scan(&existingDigest,&existingStatus,&existingMessage)
	if err==nil{if existingDigest!=bodyDigest{return 0,false,ErrConflict};if existingStatus=="ACCEPTED"&&existingMessage!=nil{return *existingMessage,true,nil};return 0,true,ErrConflict};if !errors.Is(err,pgx.ErrNoRows){return 0,false,err}
	...
	if !inserted{
		var otherDigest,otherStatus string;var otherMessage *int64;if err=tx.QueryRow(ctx,`SELECT body_sha256,status,message_id FROM mail_inbound_receipts WHERE event_id=$1`,eventID).Scan(&otherDigest,&otherStatus,&otherMessage);err!=nil{return 0,false,err};if otherDigest!=bodyDigest{return 0,false,ErrConflict};if otherStatus=="ACCEPTED"&&otherMessage!=nil{if err=tx.Commit(ctx);err!=nil{return 0,false,err};cleanup();committed=true;return *otherMessage,true,nil};return 0,true,ErrConflict
	}
