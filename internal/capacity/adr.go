...
FROM capacity_snapshots s JOIN capacity_benchmark_runs r ON r.run_id=s.run_id JOIN capacity_adr_decisions d ON d.snapshot_id=s.snapshot_id WHERE s.snapshot_id=$1`,snapshotID).Scan(&label,&mode,&docs,&measured,&workload,&projection,&bottlenecks,&choice,&rationale,&decidedBy,&decided);if errors.Is(err,pgx.ErrNoRows){return "",pgx.ErrNoRows};if err!=nil{return "",err}
	...
	search:=w["search"];recommendation:=RecommendChoice(b)
	return fmt.Sprintf("# ADR — Capacity decision for snapshot %d\n\n- Benchmark: %s\n- Mode: %s\n- Measured at: %s\n- Measured indexed documents: %d\n- Search QPS: %.2f\n- Search P50/P95/P99: %.2f / %.2f / %.2f ms\n- Search error rate: %.4f\n- 10M model: %s, factor %.3f, projected DB+index bytes %d\n- Bottlenecks: %s\n- Advisory recommendation: %s\n\n## Decision\n%s\n\n## Rationale\n%s\n\nDecision recorded by %s at %s. The recommendation is advisory only; this ADR records the human-approved architecture choice and no infrastructure change is applied automatically.\n",snapshotID,label,mode,measured.UTC().Format(time.RFC3339),docs,search.QPS,search.P50MS,search.P95MS,search.P99MS,search.ErrorRate,p.Kind,p.ScaleFactor,p.ProjectedTotalBytes,bottleneckCodes(b),recommendation,choice,rationale,decidedBy,decided.UTC().Format(time.RFC3339)),nil
}
