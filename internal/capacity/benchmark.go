...
func RunHTTPBenchmark(ctx context.Context,cfg HTTPBenchmarkConfig)(WorkloadMetrics,error){
	...
	for worker:=0;worker<cfg.Concurrency;worker++{go func(){defer wg.Done();for{
			if benchCtx.Err()!=nil{return};i:=sequence.Add(1)-1;target:=cfg.URLs[int(i%uint64(len(cfg.URLs)))];req,err:=http.NewRequestWithContext(benchCtx,http.MethodGet,target,nil);if err!=nil{errorsCount.Add(1);requests.Add(1);continue};req.Header.Set("Accept","application/json")
			began:=time.Now();resp,err:=client.Do(req);latency:=time.Since(began);requests.Add(1);failed:=err!=nil
			if err==nil{_,readErr:=io.Copy(io.Discard,io.LimitReader(resp.Body,cfg.MaxBodyBytes+1));_ = resp.Body.Close();if readErr!=nil||resp.StatusCode<200||resp.StatusCode>=300{failed=true}}
			if failed{errorsCount.Add(1)}
