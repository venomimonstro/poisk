...
func TestEnqueueIndexEventIsIdempotent(t *testing.T) {
	...
	if err := pool.QueryRow(ctx,
		`SELECT count(*) FROM index_outbox WHERE entity_type='WEB_DOCUMENT' AND entity_id=42 AND entity_version=3`,
	).Scan(&count); err != nil {
...
func TestSameVersionCannotChangeOperation(t *testing.T) {
	...
	if _, err := repo.Enqueue(ctx, "WEB_DOCUMENT", 7, 4, "UPSERT", time.Now()); err != nil {
		t.Fatalf("enqueue original event: %v", err)
	}
	if _, err := repo.Enqueue(ctx, "WEB_DOCUMENT", 7, 4, "DELETE", time.Now()); !errors.Is(err, ErrVersionConflict) {
...
func TestNewerVersionSupersedesOlderPendingEvent(t *testing.T) {
	...
	oldID, err := repo.Enqueue(ctx, "WEB_DOCUMENT", 99, 1, "UPSERT", time.Now())
	if err != nil {
		t.Fatalf("enqueue old version: %v", err)
	}
	newID, err := repo.Enqueue(ctx, "WEB_DOCUMENT", 99, 2, "DELETE", time.Now())
	if err != nil {
		t.Fatalf("enqueue new version: %v", err)
	}

	events, err := repo.Lease(ctx, "indexer", 10, 30)
	if err != nil {
		t.Fatalf("lease: %v", err)
	}
	if len(events) != 1 || events[0].ID != newID || events[0].EntityVersion != 2 {
		t.Fatalf("expected only newest version leased, got %+v", events)
	}

	var oldStatus string
	if err := pool.QueryRow(ctx, `SELECT status FROM index_outbox WHERE id=$1`, oldID).Scan(&oldStatus); err != nil {
		t.Fatalf("read old event: %v", err)
	}
	if oldStatus != "SUPERSEDED" {
		t.Fatalf("expected old event SUPERSEDED, got %s", oldStatus)
	}
}
...
func TestNewVersionWaitsForAlreadyLeasedOldVersion(t *testing.T) {
	...
	if _, err := repo.Enqueue(ctx, "WEB_DOCUMENT", 55, 1, "UPSERT", time.Now()); err != nil {
		t.Fatalf("enqueue v1: %v", err)
	}
	first, err := repo.Lease(ctx, "worker-a", 1, 30)
	if err != nil || len(first) != 1 {
		t.Fatalf("lease v1: len=%d err=%v", len(first), err)
	}
	if _, err := repo.Enqueue(ctx, "WEB_DOCUMENT", 55, 2, "UPSERT", time.Now()); err != nil {
		t.Fatalf("enqueue v2: %v", err)
	}

	second, err := repo.Lease(ctx, "worker-b", 1, 30)
	if err != nil {
		t.Fatalf("concurrent lease: %v", err)
	}
	if len(second) != 0 {
		t.Fatalf("new version must wait while old version is leased, got %+v", second)
	}

	if err := repo.MarkProcessed(ctx, first[0].ID, "worker-a"); err != nil {
		t.Fatalf("finish v1: %v", err)
	}
	third, err := repo.Lease(ctx, "worker-b", 1, 30)
	if err != nil || len(third) != 1 || third[0].EntityVersion != 2 {
		t.Fatalf("expected v2 after v1 completion, events=%+v err=%v", third, err)
	}
}
