...
func (m Monitor) Check() (Snapshot,error){
	...
	disk,err:=diskUsage(m.Path);if err!=nil{return Snapshot{},err};memory,err:=memoryUsage();if err!=nil{return Snapshot{},err}
...
func (m Monitor) Run(ctx context.Context,interval time.Duration)error{
	if interval<=0{interval=5*time.Second};ticker:=time.NewTicker(interval);defer ticker.Stop()
	for{
		snapshot,err:=m.Check()
		if err!=nil{snapshot=Snapshot{State:Critical,CheckedAt:time.Now().UTC()}}
		if persistErr:=m.Persist(ctx,snapshot);persistErr!=nil&&ctx.Err()!=nil{return ctx.Err()}
