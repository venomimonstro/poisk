...
func (r Repository) Stage(ctx context.Context,version,buildSHA string,requiredSchema int64,mapVersion,configHash,actor string)(Manifest,error){
	version=strings.TrimSpace(version);buildSHA=strings.TrimSpace(buildSHA);mapVersion=strings.TrimSpace(mapVersion);configHash=strings.ToLower(strings.TrimSpace(configHash));actor=strings.TrimSpace(actor)
	if r.DB==nil||version==""||len(version)>128||buildSHA==""||len(buildSHA)>128||requiredSchema<=0||len(configHash)!=64||actor==""{return Manifest{},ErrPreflight}
	var out Manifest
	err:=r.DB.QueryRow(ctx,`INSERT INTO app_releases(version,build_sha,required_schema_version,map_version,web_index_schema,organization_index_schema,address_index_schema,config_hash)
...
RETURNING release_id,version,build_sha,required_schema_version,COALESCE(map_version,''),web_index_schema,organization_index_schema,address_index_schema,config_hash,status`,version,buildSHA,requiredSchema,mapVersion,indexmanticore.WebSchemaVersion,indexmanticore.OrganizationsSchemaVersion,indexmanticore.AddressesSchemaVersion,configHash).Scan(&out.ID,&out.Version,&out.BuildSHA,&out.RequiredSchemaVersion,&out.MapVersion,&out.WebIndexSchema,&out.OrganizationIndexSchema,&out.AddressIndexSchema,&out.ConfigHash,&out.Status)
	if errors.Is(err,pgx.ErrNoRows){return Manifest{},ErrPreflight};if err!=nil{return Manifest{},err};_,_=r.DB.Exec(ctx,`INSERT INTO release_events(release_id,action,actor,details) VALUES($1,'STAGE',$2,'{}')`,out.ID,actor);return out,nil
}
